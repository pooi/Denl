...
if __name__ == "__main__":
  ...
  detail_graph_dir = os.path.join(detail_dir, hierarchical_label, "retrained_graph.pb")
  detail_labels_dir = os.path.join(detail_dir, hierarchical_label, "retrained_labels.txt")
  ...
  with tf.Session(graph=detail_graph) as sess:
    detail_results = sess.run(detail_output_operation.outputs[0],
                      {detail_input_operation.outputs[0]: t})
  
  detail_results = np.squeeze(detail_results)
  detail_top_k = detail_results.argsort()[-5:][::-1]

  for i in detail_top_k:
    print(detail_labels[i], detail_results[i])
